import { SNOWMAN } from '../config/constants.js';
...
export default class Snowman extends Phaser.Physics.Arcade.Sprite
{
    constructor (scene, track, size)
    {
        ...
        this.size = size;
        this.speed = SNOWMAN.SPEED;
        
        // 敵タイプ（教育用）
        // シーンのactiveEnemyTypesから選択して、ボールタイプと対応させる
        this.enemyType = Phaser.Utils.Array.GetRandom(scene.activeEnemyTypes);

        // 敵タイプ表示（教育用）
        this.typeText = scene.add.text(this.x, this.y - 120, this.enemyType, {
            fontSize: '16px',
            color: '#ffffff',
            padding: { x: 4, y: 2 }
        }).setOrigin(0.5);
        ...
        if (scene.isColorHintEnabled) {
            const color = CRITICAL_MAP[this.enemyType].color;
            this.typeText.setBackgroundColor(Phaser.Display.Color.IntegerToColor(color).rgba);
        }
    }
    ...
    start ()
    {
        ...
        this.chooseEvent = this.time.delayedCall(
            Phaser.Math.Between(SNOWMAN.MIN_SPAWN_DELAY, SNOWMAN.MAX_SPAWN_DELAY),
            this.chooseAction, [], this
        );
    }
    ...
    preUpdate (time, delta)
    {
        ...
        // 敵タイプ名を常に頭上に追従
        this.typeText.setPosition(this.x, this.y - 120);

        // this.label.setPosition(this.x, this.y - 40);

        if (this.x >= SNOWMAN.GOAL_X)
        {
            this.stop();
